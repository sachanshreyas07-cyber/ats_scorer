...
def render() -> None:
    ...
    <div class="glass-card animate-fade-up" style="margin-bottom: 2rem;">
        <h2 style="margin: 0; background: linear-gradient(135deg, #FFFFFF 50%, var(--primary) 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-weight: 700;">🎯 ATS Scoring Hub</h2>
        <p style="color: #94A3B8; margin: 0.25rem 0 0 0;">Upload your resume—and optionally a target job description—to run deep compatibility checks.</p>
    </div>
    ...
    with st.sidebar:
        ...
        st.info(
            "💡 **General ATS Score**: Checks resume structure, grammar, content quality, and unvalidated skill claims.\n\n"
            "🎯 **JD Comparison**: Matches your resume details directly against a pasted job description to show keyword and skills gaps."
        )
    ...
    if not resume_file:
        ...
        <div class="custom-alert alert-info">
            ...
            <div>
                <strong>Get started:</strong> Upload your resume file above to begin the AI scoring pipeline.
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    if not access_token:
        ...
        <div class="custom-alert alert-warning">
            <span style="font-size: 1.2rem;">⚠️</span>
            <div>
                <strong>Authentication Required:</strong> Please sign in or create an account in the sidebar to run the analysis.
            </div>
        </div>
        """, unsafe_allow_html=True)
        return
