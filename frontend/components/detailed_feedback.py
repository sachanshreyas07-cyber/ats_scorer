...
def _render_issue(issue: Dict[str, Any]) -> None:
    ...
    alert_class = "alert-danger" if level in ("critical", "high") else "alert-warning" if level == "medium" else "alert-success"
    ...
    st.markdown(
        f"""
        <div class="custom-alert {alert_class} animate-fade-up" style="margin-bottom: 0.5rem; padding: 0.85rem 1.25rem;">
            <span style="font-size: 1.1rem;">{icon}</span>
            <div style="flex: 1;">
                <strong style="font-size: 0.95rem;">{title}</strong>
                <span style="opacity: 0.85; margin-left: 0.5rem; font-size: 0.85rem;">| Impact: {impact}</span>
            </div>
        </div>
        """,
    ...
    with st.expander("Details & Remediation Steps", expanded=False):
        if explanation:
            st.markdown(f"❓ **What's happening:** {explanation}")
        if where:
            st.markdown(f"📍 **Where it appears:** {where}")
        if how_to_fix:
            st.markdown(f"🛠️ **How to fix:** {how_to_fix}")
        if action_items:
